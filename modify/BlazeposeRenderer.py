...
class BlazeposeRenderer:
    ...
    def waitKey(self, delay=1, body=None):
        if self.show_fps:
            self.tracker.fps.draw(self.frame, orig=(50,50), size=1, color=(240,180,100))
        if body is not None:
            cv2.line(self.frame, (int(body.rect_points[0][0]), int(body.rect_points[0][1])),
                     (int(body.rect_points[1][0]), int(body.rect_points[1][1])), (0, 0, 255), 4, 4)
            cv2.line(self.frame, (int(body.rect_points[1][0]), int(body.rect_points[1][1])),
                     (int(body.rect_points[2][0]), int(body.rect_points[2][1])), (0, 0, 255), 4, 4)
            cv2.line(self.frame, (int(body.rect_points[2][0]), int(body.rect_points[2][1])),
                     (int(body.rect_points[3][0]), int(body.rect_points[3][1])), (0, 0, 255), 4, 4)
            cv2.line(self.frame, (int(body.rect_points[3][0]), int(body.rect_points[3][1])),
                     (int(body.rect_points[0][0]), int(body.rect_points[0][1])), (0, 0, 255), 4, 4)
            cv2.putText(self.frame, f"*", (int(body.rect_x_center_a), int(body.rect_y_center_a)), cv2.FONT_HERSHEY_PLAIN, 1,
                        (0, 0, 255), 4)
