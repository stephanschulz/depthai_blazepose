...
#        self.device.startPipeline(self.create_pipeline(), device_info)
        ...
        # Define data queues 
#        if not self.laconic:
        self.q_mono_manip = self.device.getOutputQueue(name="mono_manip_out", maxSize=1, blocking=False)
#        self.q_video = self.device.getOutputQueue(name="right_out", maxSize=1, blocking=False)
#        self.q_video = self.device.getOutputQueue(name="left_out", maxSize=1, blocking=False)
...
#        cam.preview.link(pre_lm_manip.inputImage)
    ...
    def next_frame(self):
        ...
        if self.laconic:
            mono_manip_frame = np.zeros((self.frame_size, self.frame_size, 3), dtype=np.uint8)
#            right_frame = np.zeros((self.frame_size, self.frame_size, 1), dtype=np.uint8)
        else:
            in_mono_video = self.q_mono_manip.get()
            mono_manip_frame = in_mono_video.getCvFrame()       
